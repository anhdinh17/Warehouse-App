...
final class DatabaseManager {
    ...
    public func readItems(completion: @escaping ([String:Any]?,String?)->Void){
        ...
        Database.database().reference(withPath: "\(username)/Items").observe(.childAdded){snapshot in
    ...
    public func updateNewQuantity(item: String,id: String,newQuantity: Int, completion: @escaping (Bool)->Void){
        ...
        database.child(username).child("Items").child(id).setValue(newQuantity){ error,_ in
            guard error == nil else {
                completion(false)
                return
            }
    ...
    public func storeTransaction(receiverName: String,item: String, quantity: Int, date: String, completion: @escaping(Bool)->Void){
        ...
        database.child(username).child("Receivers").observeSingleEvent(of: .value) { [weak self] snapshot in
            guard var receiverDictionary = snapshot.value as? [String:Any] else {
                // if there's no "Receivers" yet
                self?.database.child(username).child("Receivers").setValue(
                    [
                        receiverName: [
                            "Item": item,
                            "Quantity": quantity,
            ...
            self?.database.child(username).child("Receivers").setValue(receiverDictionary){(error,_) in
                guard error == nil else {
                    completion(false)
                    return
                }
